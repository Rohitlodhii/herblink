...
export async function createHerbInventory(req: AuthenticatedRequest, res: Response) {
  try {
...
export async function listHerbInventories(req: AuthenticatedRequest, res: Response) {
  try {
    const userId = req.userId;
    if (!userId) {
      return res.status(401).json({ msg: "Unauthorized" });
    ...
    const herbInventories = manufacturer.products.flatMap((product) =>
      product.Inventories.map((inventory) => inventory.herbInventory).filter(Boolean)
    );
...
export async function getHerbInventoryById(req: AuthenticatedRequest, res: Response) {
  try {
    const userId = req.userId;
    const { id } = req.params;
...
export async function updateHerbInventory(req: AuthenticatedRequest, res: Response) {
  try {
    const userId = req.userId;
    const { id } = req.params;
