...
export async function signUpLab( req :Request ,res :Response ){
    try {
        ...
        logger.error("lab_signup_error", { error: (error as Error).message, stack: (error as Error).stack });
...
export async function loginLab( req :Request ,res : Response ) {
    try {
       ...
       if(!checkPassword){
        ...
        logger.error("lab_login_error", { error: (error as Error).message, stack: (error as Error).stack });
        return res.status(400).json({
            msg : "Internal Server error"
        })
